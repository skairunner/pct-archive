...
timefmt = '%Y-%m-%d %H:%M:%S'
async def get_logs():
    snipchannel = client.get_channel(CHANNEL_ID)
    reportchannel = client.get_channel(SNIP_DIFF_REPORT_CHANNEL)
    data = []
    counter = 0

    # get last msgid from discord
    lastmsgid = DiscordMessage.objects.order_by('-timestamp').first().messageid
    lastmsg = await client.get_message(snipchannel, lastmsgid)


    kwargs = {'after': lastmsg}

    # Gets up to 100 messages since last importing.
    async for msg in client.logs_from(snipchannel, limit=100, **kwargs, reverse=True):
        data.append({
    ...
    if len(data) != 0:
        ...
        await client.send_message(reportchannel, content)
